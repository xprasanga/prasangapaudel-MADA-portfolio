...
ggplot(pred_data, aes(x = truth, y = .pred)) +
  ...
  theme_bw()
```
The datapoints are still far away from the 45-degree line as observed in Linear Regression Model.
...
We can observe that, even with CV, we get a similar result that we observed without CV. The RMSE value shows a sharp rise as the penalty is increased.
